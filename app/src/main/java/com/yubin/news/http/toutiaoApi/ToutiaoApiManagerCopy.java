...
import com.yubin.news.model.toutiaoApi.ToutiaoSuperGalleryBean;
...
public class ToutiaoApiManagerCopy {
...
//        String url="http://www.toutiao.com/api/pc/feed/?category=gallery_detail&max_behot_time="+behot_time+"&as=A1550911339D015&cp=5913FDA05125BE1";
...
//        }
//        LogUtil.i("word="+word);
//        String url="http://www.toutiao.com/api/pc/feed/?category="+word+"&max_behot_time=0&as=A15569E16C7029C&cp=591CC0C2599CDE1";
...
//        String url="http://www.toutiao.com/api/pc/feed/?category=gallery_detail";
        LogUtil.i("url=" + url);
...
//                        Type typeToken=new TypeToken<QuestionDetailBean<ChoiceQuestionBean>>(){}.getType();
//                        QuestionDetailBean<ChoiceQuestionBean> questionDetailBean=gson.fromJson(response,typeToken);
//                        ChoiceQuestionBean choiceQuestionBean=questionDetailBean.getQuestion();
            ...
            @Override
            public void onErrorResponse(VolleyError volleyError) {
                LogUtil.i("volleyError.getMessage()=" + volleyError.getMessage());
...
//                headers.put("Accept-Language", "zh-CN,zh;q=0.8");
//                headers.put("Cache-Control", "max-age=0");
//                headers.put("Connection", "max-age=0");
//                headers.put("Host", "http://a3.pstatp.com");
//                headers.put("Upgrade-Insecure-Requests", "1");
//                headers.put("Content-Type", "application/x-www-form-urlencoded");
//                headers.put("User-Agent", "Mozilla/5.0 (Linux; Android 6.0; Nexus 5 Build/MRA58N) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/55.0.2883.11 Mobile Safari/537.36");
//                LogUtil.i("headers=" + headers);
                return headers;
            }
        };
...
//        jsonObjectRequest.setShouldCache(true);
    ...
    public static void getImageStoryGallery2(String groupId, final ToutiaoGetImageStoryGalleryListener listener) {
        String url = "http://a3.pstatp.com/article/full/16/1/" + groupId + "/6407645878584082946/1/0/?iid=10109443249&device_id=36240811078&ac=wifi&channel=tengxun&aid=13&app_name=news_article&version_code=612&version_name=6.1.2&device_platform=android&ab_version=120430%2C124684%2C123181%2C112577%2C125848%2C125887%2C115758%2C114037%2C122834%2C113607%2C123190%2C125863%2C123727%2C114105%2C125698%2C113608%2C125451%2C125720%2C125503%2C125174%2C118016%2C104320%2C123178%2C121512%2C112578%2C124050%2C125393%2C125907%2C122948%2C123123%2C31207%2C125872%2C121005%2C125528%2C114338%2C123717&ab_client=a1%2Cc4%2Ce1%2Cf2%2Cg2%2Cf7&ab_feature=102749%2C94563&abflag=3&ssmix=a&device_type=A0001&device_brand=Sony&language=zh&os_api=17&os_version=4.2.2&uuid=864895026212660&openudid=40393a9a523ac06a&manifest_version_code=612&resolution=1080*1920&dpi=320&update_version_code=6124&_rticket=1494468882034";
        LogUtil.i("url=" + url);
        RequestQueue requestQueue = MyApplication.getRequestQueue();
        final Gson gson = MyApplication.getGson();
            ...
            public void onResponse(JSONObject jsonObject) {
                ...
                try {
                    ...
                    if (message.equals("success")) {
                        ...
                        ToutiaoSuperGalleryBean toutiaoSuperGalleryBean = gson.fromJson(data, ToutiaoSuperGalleryBean.class);
                        for (int i = 0; i < toutiaoSuperGalleryBean.getGallery().size(); i++) {
                            LogUtil.i("imageTitle=" + toutiaoSuperGalleryBean.getGallery().get(i).getSub_title());
                            LogUtil.i("imageurl=" + toutiaoSuperGalleryBean.getGallery().get(i).getSub_image());
                        }
                        listener.onResult(toutiaoSuperGalleryBean.getGallery());
                    } else {
                        LogUtil.i("data=null");
                        listener.onError(null);
                    }
                ...
                try {
                    ...
                    if (message.equals("success")) {
                        ...
                        for (int i = 0; i < toutiaoRelatedGalleryArrayBean.getRelatedGalleryList().size(); i++) {
                            LogUtil.i("for=success");
                            LogUtil.i("galleryTitle=" + toutiaoRelatedGalleryArrayBean.getRelatedGalleryList().get(i).getTitle());
                            LogUtil.i("galleryUrl=" + toutiaoRelatedGalleryArrayBean.getRelatedGalleryList().get(i).getArticle_url());
                        }
                    ...
                    LogUtil.i("数据解析错误！" + ex.getStackTrace());
                    listener.onError("数据解析错误！");
                    ex.printStackTrace();
                }
            }
    ...
    private static String getAGoodCachePathyUrl(String url){
        String filepath;
        if(url.contains("/")){
...
//        }
        ...
        final Gson gson = MyApplication.getGson();
        if(!NetworkUtil.isNewworkAvailable(context)){
            try {
                LogUtil.i("readFile=" + filepath);
                String dataStr= CacheUtil.readFile(filepath).toString();
//                convertSomeNewsJson(dataStr,gson,listener,filepath);
            }catch (Exception ex){
                ex.printStackTrace();
            }
        }else{
...
//                        convertSomeNewsJson(dataStr,gson,listener,filepath);
     ...
     * @param filepath
     */
    public static  void convertSomeNewsJson(String dataStr,Gson gson,boolean getNetData,ToutiaoGetSomeNewsListener listener,String filepath){
        List<TouTiaoNewsBean> datalist = new ArrayList<>();
        try {
            CacheUtil.saveFile(dataStr,filepath);
            LogUtil.i("saveFile=" + filepath);
            dataStr = "{\"data\":" + dataStr + "}";
            LogUtil.i("data=" + dataStr);
            ToutiaoNewsArrayBean toutiaoNewsArrayBean = gson.fromJson(dataStr, ToutiaoNewsArrayBean.class);
            LogUtil.i("toutiaoNewsArrayBean=success");
            ...
            listener.onResult(datalist,getNetData,false);
        } catch (Exception ex) {
            LogUtil.i("数据解析错误！" + ex.toString());
            listener.onError("数据解析错误！");
            listener.onResult(datalist,getNetData,false);
            ex.printStackTrace();
        }
